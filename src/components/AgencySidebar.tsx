...
import type { AgencyPage } from "@/lib/tour-agency";
import {
  Chevron,
  DealsIcon,
  MoneyIcon,
...
type Item = { key: string; label: string; pages?: AgencyPage[]; badge?: string };
type Group = {
  key: string;
  label: string;
...
export function AgencySidebar({ active }: { active: AgencyPage }) {
  ...
  return (
    <aside className="hidden w-[225px] shrink-0 flex-col border-r border-ft-border bg-ft-surface md:flex">
      ...
      <div>
        <div
          className={`flex items-center gap-2.5 px-4 py-2.5 text-[14px] font-semibold ${
            refsActive
              ? "border-l-[3px] border-ft-primary bg-ft-surface-muted pl-[13px] text-ft-primary"
              : "border-l-[3px] border-transparent border-t border-t-ft-border text-ft-text-on-nav"
          }`}
        >
          <Chevron
            open={refsActive}
            className={
              refsActive ? "text-ft-primary" : "text-ft-text-on-nav/50"
            }
          />
          <ReferenceIcon
            className={
              refsActive ? "text-ft-primary" : "text-ft-text-on-nav"
            }
          />
          Справочники
        </div>
