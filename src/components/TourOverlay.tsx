...
import { tourScript, type TourStep } from "@/lib/tour";
...
export function TourOverlay({
  ...
  const step: TourStep | undefined = tourScript[stepIndex];
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-40">
      ...
      <div
        className="pointer-events-auto absolute w-[360px] max-w-[calc(100vw-32px)] rounded-lg bg-white p-4 shadow-2xl ring-1 ring-slate-200"
        style={{ top: tooltipPos.top, left: tooltipPos.left }}
      >
        <div className="flex items-start justify-between gap-2">
          <div className="flex items-center gap-2">
            <span className="rounded bg-blue-100 px-2 py-0.5 text-[11px] font-semibold text-blue-700">
              Шаг {stepIndex + 1} из {tourScript.length}
            </span>
        ...
        <div className="mt-3 flex items-center justify-between gap-2">
          ...
          >
            {step.cta ?? (stepIndex === tourScript.length - 1 ? "Завершить" : "Дальше →")}
          </button>
        </div>
      </div>
    </div>
...
function pageLabel(page: TourStep["page"]) {
  switch (page) {
    case "svodka":
      return "Сводка";
    case "deals":
