import { universalAccounts } from "@/lib/universal-data";
import { formatRub } from "@/lib/provenance";

export function UniversalVirtualAccountPage() {
  return (
    <div className="space-y-5">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-600">
            Виртуальные счета — для разделения денег по назначению. Гарантийные
            удержания не путаются с заработанными деньгами.
          </p>
        </div>
        <button
          type="button"
      ...
      <div className="rounded-md border border-emerald-200 bg-emerald-50/40 p-4">
        ...
        <p className="mt-2 text-sm leading-relaxed text-slate-800">
          ...
          Чтобы быть честным по отношению к себе, переведите сумму на
          специальный счёт». Тот же принцип применяется к гарантийным удержаниям
          у монтажных и сервисных компаний.
        </p>
        <a
        ...
        >
          help.fintablo.ru/article/1834 — про виртуальные счета →
        </a>
      </div>
    </div>
  );
